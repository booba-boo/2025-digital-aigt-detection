...
3. **학습 및 추론 구조**:
   ...
   * 데이콘 토크 Q&A에서 학습과 추론 파일을 별개로 제출하는 것을 권장하셨지만, 최종 private score를 기록한 코드는 학습과 추론이 하나의 코드로 연결된 형태였습니다. 이에 권장하신 방법에 따라 학습 코드와 추론 코드를 따로 분리 후 연습 모드에서 결과를 제출해보니 점수가 동일하지 않고 오히려 올라가는 현상이 발생했습니다. 학습/추론 코드를 분리하여 결과를 제출하더라도 리더보드 순위 변동에는 영향이 없겠지만, private score 재현을 1순위로 고려하여서 부득이하게 현재 private score를 기록했던 원래 코드로 학습과 추론 코드가 연결되어 있는 형태로 제출하게 되었습니다.
